...
class SelectTrafficByTimePrefer extends AbstractPrefer<IFinalTicket> {
    ...
    async markScoreProcess(tickets:IFinalTicket[]):Promise<IFinalTicket[]> {
        ...
        tickets.forEach( (v) => {
            if (v.type == ETrafficType.TRAIN && /^[gc]/i.test(v.No)) {
                highTrains.push(v);
                return;
            }
            if (v.type == ETrafficType.TRAIN && /^[d]/i.test(v.No)) {
                trains2.push(v);
                return;
            }
        ...
        let gainScoreTraffic: ETrafficType = null;
        ...
        if (trains[midIdx].duration <= self.selectTrainDuration) {
            ...
            //火车加分
            gainScoreTraffic = ETrafficType.TRAIN;
            ...
            gainScoreTraffic = ETrafficType.PLANE;
        } else {
